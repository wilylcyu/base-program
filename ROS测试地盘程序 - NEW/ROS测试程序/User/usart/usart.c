...
void USART2_Config(void)                                    //初始化 配置USART2
{
   ...
   RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOD | RCC_APB2Periph_AFIO, ENABLE);   //打开PA端口时钟
	 GPIO_PinRemapConfig(GPIO_Remap_USART2,ENABLE);

	 //将USART2 Tx（发送脚）的GPIO配置为推挽复用模式，复用后直接接到相应引脚上，接到usart2 TX ,RX上不可用
	 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_5;				          //选定哪个IO口 现选定PD5
	 GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;           //复用推挽输出
	 GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		      //设定IO口的输出速度为50MHz
	 GPIO_Init(GPIOD, &GPIO_InitStructure);    				        //初始化GPIOD

	 //将USART2 Rx（接收脚）的GPIO配置为浮空输入模式														  
	 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6;				          //选定哪个IO口 现选定PD6
	 GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;	            //上拉输入
	 GPIO_Init(GPIOD, &GPIO_InitStructure);                    //初始化GPIOD
